...
HTTP-адаптер проверяет успешность ответа, а ошибки приводятся к единому сериализуемому формату. Основной список и страница задачи различают первоначальную загрузку, фоновое обновление, ошибку и успешное состояние. При ошибке загрузку можно повторить, а уже полученные данные сохраняются на экране при неудачном фоновом запросе.

Ошибки добавления, изменения и удаления задач показываются через toast-уведомления. Удаление всех задач подтверждается с помощью доступного диалога на базе Radix UI.
...
- унифицировать обработку ошибок серверного и локального адаптеров;
- добавить runtime-валидацию ответов HTTP API;
